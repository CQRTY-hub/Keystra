...
**Status:** geen van de resterende punten is door een jurist bevestigd. Overal waar dit document een vraag noemt, staat in de bijbehorende brontekst (`terms.en/nl.ts`, `contact.en.ts` via de contactpagina) letterlijk dezelfde vraag, zichtbaar gemarkeerd als `[TE BEVESTIGEN DOOR JURIST: ...]` / `[TO BE CONFIRMED BY LAWYER: ...]` — dit bestand verzamelt ze alleen op één plek voor het gesprek. Punt 3 (btw) is inmiddels beantwoord door de boekhouder — zie "Opgelost" onderaan, de markering is uit de teksten verwijderd.

De bestaande waarschuwing dat geen enkele tekst op deze site al door een Belgische jurist is nagekeken (`LEGAL_REVIEW_NOTICE_NL` / `LEGAL_REVIEW_NOTICE_EN`) blijft onverkort van kracht — ook voor de teksten die deze vragen niet raken.
...
### 3. Geen apart modelformulier voor herroeping
...
### 3. No separate model withdrawal form
...
### Btw: vrijstellingsregeling kleine ondernemingen — beantwoord 2026-09-01
...
- **Wat er open stond:** of Keystra terecht onder de vrijstellingsregeling kleine ondernemingen valt (art. 56bis Btw-Wetboek) en dus geen btw aanrekent.
- **Antwoord van de boekhouder:** bevestigd — geen btw zolang de totale jaaromzet onder €25.000 blijft **en** de buitenlandse (niet-Belgische EU-)omzet onder €10.000. Boven die drempels geldt een EU-vrijstellingsregeling met kwartaalrapportage per lidstaat.
- **Wat er gebouwd is naar aanleiding daarvan:** landregistratie bij checkout en op de factuur, een drempelteller in het adminpaneel (met 80%-waarschuwing) en een kwartaal-exportfunctie per land — zie `src/lib/vat-thresholds.ts` en `src/lib/vat-export.ts`. De `[TE BEVESTIGEN DOOR JURIST/BOEKHOUDER: ...]`-markering is uit `terms.en.ts` / `terms.nl.ts` verwijderd.

### VAT: small-business exemption scheme — resolved 2026-09-01
...
- **What was open:** whether Keystra correctly falls under the small-business VAT exemption scheme (art. 56bis of the Belgian VAT Code) and therefore doesn't charge VAT.
- **Accountant's answer:** confirmed — no VAT as long as total annual turnover stays under €25,000 **and** foreign (non-Belgian EU) turnover stays under €10,000. Above those thresholds, an EU exemption scheme applies with quarterly per-member-state reporting.
- **What got built as a result:** country capture at checkout and on the invoice, a threshold tracker in the admin panel (with an 80% warning), and a quarterly per-country export — see `src/lib/vat-thresholds.ts` and `src/lib/vat-export.ts`. The `[TO BE CONFIRMED BY LAWYER / ACCOUNTANT: ...]` marker has been removed from `terms.en.ts` / `terms.nl.ts`.
